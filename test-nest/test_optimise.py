...
def test_gradient_optimiser_train_simple():
    ...
    assert np.allclose(l.get_weights(), np.array([[1, 0.9]])) # No change
    xs = np.array([[1]])
    ys = np.array([[0, 1]])
    m, y = optimiser.train(model, xs, ys)
    assert np.allclose(y, np.array([[1]]))
    assert np.allclose(l.get_weights(), np.array([[0.9, 0.9]])) # Strengthen right
    xs = np.array([[1]])
    ys = np.array([[1, 0]])
    m, y = optimiser.train(model, xs, ys)
    assert np.allclose(y, np.array([[1]]))
    assert np.allclose(l.get_weights(), np.array([[0.9, 0.8]])) # Strengthen left
